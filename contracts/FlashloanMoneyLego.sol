...
contract FlashloanMoneyLego is FlashLoanReceiverBase {
    ...
    function executeOperation(
        ...
        uint256 batPurchased =
            exchangeBforLoanAsset.tokenToTokenSwapInput(_amount, 1, 1, deadline, BAT_ADDRESS);
        ...
        uint256 reserveAssetPurchased =
            exchangeAforBAT.tokenToTokenSwapInput(batPurchased, 1, 1, deadline, RESERVE_ADDRESS);

        uint256 amount = _amount;

        uint256 totalDebt = amount.add(_fee);
    ...
    function initateFlashLoan(address assetToFlashLoan, uint256 amountToLoan) external {
        bytes memory data = '';
        ...
        // Ask for a flashloan
        lendingPool.flashLoan(address(this), assetToFlashLoan, amountToLoan, data);
