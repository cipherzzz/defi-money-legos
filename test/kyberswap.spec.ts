...
const gasLimit = process.env.GAS_LIMIT;
    ...
    it('swaps Eth for Dai', async function() {
        ...
        let tx = await contract.ethToToken(legos.erc20.dai.address, {value: parseEther(".1"), gasLimit});
        
        const daiBalanceAfter = await dai.balanceOf(wallet.address);
        assert(daiBalanceAfter.gt(daiBalanceBefore), "Expected die balance after transfer to be greater")
    });
}); 