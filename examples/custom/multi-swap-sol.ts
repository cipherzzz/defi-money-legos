...
const kyberSwap = new ethers.Contract(
  KyberSwapInterface.networks[process.env.NETWORK_ID].address,
  KyberSwapInterface.abi,
  wallet
);
...
const uniswapV1 = new ethers.Contract(
  UniswapV1Interface.networks[process.env.NETWORK_ID].address,
  UniswapV1Interface.abi,
  wallet
);
async function swapEtherForTokenOnUniswapV1(destinationToken, sourceTokenAmount) {
  const tx = await uniswapV1.ethToToken(destinationToken, {
    value: parseEther(sourceTokenAmount),
    gasLimit,
  });
  const receipt = await tx.wait();
